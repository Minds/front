...
export class ComposerAudienceSelectorService {
  ...
  private readonly groupsPagingToken$: BehaviorSubject<number> =
    new BehaviorSubject<number>(0);

  /** Next paging token. */
  private readonly groupsNextPagingToken$: BehaviorSubject<number> =
    new BehaviorSubject<number>(0);

  /** Whether groups fetch request is in progress */
  public groupsLoading$: BehaviorSubject<boolean> =
  ...
  public readonly shareToGroupMode$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);

  /**
   * Groups page - change offset to advance page.
   */
  public readonly groupsPage$: Observable<SelectableEntity[]> =
