...
describe('PlusMarketingComponent', () => {
  ...
  it('Should load using the proper endpoint, plus is false', () => {
    comp.user = {
      "guid": "732337264197111809",
      "type": "user",
      "subtype": false,
  ...
  it('Should load using the proper endpoint, plus is true on response', fakeAsync(() => {
    comp.user = {
      "guid": "732337264197111809",
      "type": "user",
      "subtype": false,
    ...
    fixture.detectChanges();
    tick();
    comp.openSubscription();

    fixture.detectChanges();
    expect(comp.showSubscription).toBe(true);
    expect(comp.isPlus()).toBe(true);
    expect(clientMock.get.calls.mostRecent().args[0]).toEqual('api/v1/plus');
  }));
