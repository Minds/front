 ...
 * Progress should not be continued until the user has either copied or downloaded their seed.
 */
@Component({
...
export class SettingsTwoFactorRecoveryCodeComponent
  ...
  /**
   * Gets recovery code from service.
  ...
  private copyToClipboard(): void {
    this.subscriptions.push(
      this.recoveryCode$.pipe(take(1)).subscribe((recoveryCode: string) => {
        const selBox = document.createElement('textarea');
        selBox.style.position = 'fixed';
        selBox.style.left = '0';
        selBox.style.top = '0';
        selBox.style.opacity = '0';
        selBox.value = recoveryCode;
