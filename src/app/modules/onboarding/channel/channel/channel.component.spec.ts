...
describe('ChannelSetupOnboardingComponent', () => {
    ...
    TestBed.configureTestingModule({
      ...
      declarations: [
        MockComponent({
          ...
          inputs: ['object', 'src', 'editMode', 'waitForDoneSignal', 'icon', 'showPrompt'],
          outputs: ['added']
        }),
  ...
  it("should allow to update the user's display name", fakeAsync(() => {
    const input: DebugElement = fixture.debugElement.query(By.css('input#display-name'));
    ...
    expect(clientMock.post.calls.mostRecent().args[1]).toEqual({ name: 'test' });
  }));


  it("should allow to update the user's brief description", fakeAsync(() => {
    const input: DebugElement = fixture.debugElement.query(By.css('input#description'));
    expect(input).not.toBeNull();
    ...
    expect(clientMock.post).toHaveBeenCalled();
    expect(clientMock.post.calls.mostRecent().args[1]).toEqual({ briefdescription: '' });
  }));
