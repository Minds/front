...
@Component({
  selector: 'm-comments__threadV2',
  templateUrl: 'thread.component.html',
  styleUrls: ['thread.component.ng.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CommentsThreadV2Component implements OnInit, AfterViewInit {
  @Input() parent;
  @Input() entity;
  @Input() entityGuid;
  ...
  // TODO: use the inProgress state of commentsQuery
  ...
  commentsQuery = this.apiResource.query<any, any>('', {
    ...
    updateState: (newState, oldState) => {
      ...
      const descending = this.direction === 'desc';
    ...
    this.socketSubscriptions.comment = this.sockets.subscribe(
      ...
      async (entity_guid, owner_guid, guid) => {
        ...
        try {
          let comment;
          // GET A COMMENT HERE
          const comments = await this.commentsQuery
            .fetch(
              {
                limit: 1,
                reversed: false,
              ...
              {
                url: `api/v2/comments/${entity_guid}/${guid}/${parent_path}`,
              }
            )
            .data$.pipe(take(2))
            .toPromise();

          if (comments?.[0]._guid === guid) {
            comment = comments?.[0];
          }
