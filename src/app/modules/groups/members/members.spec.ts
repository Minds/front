...
describe('GroupsMembersModuleComponent', () => {
  ...
  it('should have a title', () => {
    const title = fixture.debugElement.query(
      By.css('.m-group--members .mdl-card__title-text')
    );
  ...
  it('should link to the group members list page', () => {
    const inviteLink = fixture.debugElement.query(
      By.css('.m-group--members .mdl-card__title a')
    );
    expect(inviteLink).not.toBeNull();
    expect(inviteLink.nativeElement.textContent).toContain('Invite');
    expect(inviteLink.nativeElement.href).toContain(
      `groups/profile/${group.guid}/members`
    );
  });
