...
describe('BlockchainConsoleComponent', () => {
      ...
      providers: [
        {
          ...
          useValue: MockService(BlockchainService),
        },
        {
  ...
  it('should have a title', () => {
    const h3 = fixture.debugElement.query(
      By.css('.m-blockchain--options-wallet h3')
    );
  ...
  it('should have an explanation', () => {
    const text = fixture.debugElement.query(
      By.css('.m-blockchain--options-wallet--setup p')
    );
  ...
  it('should have a form with a wallet address input, a list of local wallets and a save button', () => {
    ...
    const inputLabel = fixture.debugElement.query(
      By.css('form .m-blockchain--options--section:first-child label')
    );
    expect(inputLabel).not.toBeNull();
    expect(inputLabel.nativeElement.textContent).toContain(
      'Ethereum Wallet Address'
    );

    const input = fixture.debugElement.query(
      By.css('form .m-blockchain--options--section:first-child input')
    );
    expect(input).not.toBeNull();

    const walletsLabel = fixture.debugElement.query(
      By.css('form .m-blockchain--options--section:nth-child(2) label')
    );
    ...
    const refreshButton = fixture.debugElement.query(
      By.css('.m-blockchain--options--local-wallets--refresh')
    );
    expect(refreshButton).not.toBeNull();
    expect(refreshButton.nativeElement.textContent).toContain('autorenew');

    expect(
      fixture.debugElement.query(By.css('m-blockchain--wallet-selector'))
    ).not.toBeNull();

    const saveButton = fixture.debugElement.query(
      By.css('.m-blockchain--options--section button')
    );
