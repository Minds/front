...
export class PolygonDepositBoxContractService extends AbstractPolygonMindsContractService {
  ...
  constructor(
    ...
    if (this.polygonConfig) {
      this.depositBoxAddress = this.polygonConfig.Matic.Contracts.Tokens.MaticToken;
      this.depositBoxAbi = this.polygonChildAbi;
      this.maticChainName = this.polygonConfig.Matic.NetworkName;
    }
  }
  ...
  public async deposit(amount: number): Promise<any> {
    ...
    let depositReceipt = await this.web3Wallet.sendSignedContractMethod(
      depositBox,
      'depositERC20',
      [this.maticChainName, this.web3Wallet.config.token.address, amountWei],
      ''
    );
