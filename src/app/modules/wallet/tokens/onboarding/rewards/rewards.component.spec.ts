...
describe('TokenRewardsOnboardingComponent', () => {
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      declarations: [
        ...
        MockComponent({ selector: 'm-token--onboarding--video', inputs: ['src']}),
      ],
      imports: [
        FormsModule
      ],
  ...
  it('should show verify structure', fakeAsync(() => {
    ...
    expect(fixture.debugElement.query(By.css(`.m-token--onboarding--subtext`))).not.toBeNull();
    expect(fixture.debugElement.query(By.css(`.m-token--onboarding--slide`))).not.toBeNull();
    expect(fixture.debugElement.query(By.css(`m-token--onboarding--video`))).not.toBeNull();
  }));
  ...
  it('should call proper endpoints', fakeAsync(() => {
    ...
    tick();
    fixture.detectChanges();
    expect(clientMock.post.calls.mostRecent().args[0]).toBe('api/v2/blockchain/rewards/verify');
    expect(clientMock.post.calls.mostRecent().args[1]).toEqual({ number: 1111 });
  }));
  ...
  it('should show confirm endpoints', fakeAsync(() => {
    ...
    comp.join();
    fixture.detectChanges();
    expect(clientMock.post.calls.mostRecent().args[0]).toBe('api/v2/blockchain/rewards/confirm');
  }));

  it('should show confim endpoints and fail', fakeAsync(() => {
    spyOn(sessionMock, 'getLoggedInUser').and.returnValue({ guid: 1234, rewards: ['s'] });
    clientMock.response[`api/v2/blockchain/rewards/confirm`] = {
      "status": "error",
      "secret": "0",
    };
