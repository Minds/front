...
export class ThemeService {
  ...
  setUp(){
    window.addEventListener('storage', (event) => {
      if (event.key === 'dark_theme' && window.Minds.LoggedIn) {
        if ((event.newValue === 'true' && this.isDark.value === false) ||
  ...
  applyThemePreference(){
    ...
    if (this.session.getLoggedInUser().theme === 'dark')  
  ...
  renderTheme() {
    if (this.allowSetStorage)
      this.storage.set('dark_theme', this.isDark.value);
