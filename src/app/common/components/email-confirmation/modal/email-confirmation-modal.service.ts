...
export class EmailConfirmationModalService {
  ...
  public submitCode(code: string): Observable<ApiResponse | null> {
    // return this.api.post('api/v3/security/2fa/verify-email', { code: code })
    //   .pipe(take(1))
    return this.api
      .post('api/v2/email/confirmation/verify', { code: code })
      .pipe(take(1));
  }
}
