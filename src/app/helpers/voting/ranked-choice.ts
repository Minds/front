...
function irv(ballots, rounds) {
  ...
  const votes = Object.entries(
    ballots.reduce((_votes, [v], i, src) => {
      _votes[v[0]][0] += src[i][1];
      _votes[v[0]][1].length > 1
        ? (_votes[v[0]][1] = _votes[v[0]][1].map(
            (score, sI) => score + src[i][2][sI]
          ))
        : (_votes[v[0]][1] = src[i][2]);
      return _votes;
  ...
  const top = votesWithoutScore.reduce(
    ([n, m]: any[], [v, c]: any[]) => (c > m ? [v, c] : [n, m]),
    ['?', -Infinity]
  );
  const bottom = votesWithoutScore.reduce(
    ([n, m]: any, [v, c]: any) => (c < m ? [v, c] : [n, m]),
    ['?', Infinity]
  );

  const topCount = top[1];
  const bottomCand = bottom[0];
...
export class RankedChoiceVoting extends VotingStrategy {
  ...
  sumOfResultsBalance() {
    return this.resultsByVoteBalance().reduce((a, b) => a + b);
  }
}
