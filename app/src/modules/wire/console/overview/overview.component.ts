...
export class WireConsoleOverviewComponent {
  ...
  ready: boolean = true;
  stats: { points, points_count, points_avg, money, money_count, money_avg, tokens, tokens_count, tokens_avg } = {
    points: 0,
    points_count: 0,
    points_avg: 0,
    money: 0,
  ...
  getStats() {
    this.client.get('api/v1/wire/sums/overview/' + this.session.getLoggedInUser().guid, {
      ...
      .then(({ points = 0, points_count = 0, points_avg = 0, money = 0, money_count = 0, money_avg = 0, tokens = 0, tokens_count = 0, tokens_avg = 0 }) => {
        this.stats = {
          points,
          points_count,
          points_avg,
